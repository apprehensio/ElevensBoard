...
public class CardGameGUI extends JFrame implements ActionListener
{
    ...
    private void initDisplay()
    {
        ...
        if (numCardRows > 2) height += (numCardRows - 2) * LAYOUT_HEIGHT_INC;
        ...
        if (!board.anotherPlayIsPossible()) signalLoss();
    ...
    {
        String str = "cards/";
        if (c == null) return "cards/back1.GIF";
        str += c.rank() + c.suit();
        if (isSelected) str += "S";
        str += ".GIF";
        return str;
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource().equals(replaceButton))
        {
            ...
            for (int k = 0; k < board.size(); k++)
            {
                if (selections[k]) selection.add(new Integer(k));
            }
            ...
            if (board.isEmpty()) signalWin();
            else if (!board.anotherPlayIsPossible()) signalLoss();
